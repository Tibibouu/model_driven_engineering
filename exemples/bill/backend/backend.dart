...
// Create a Firestore document representing a car

Future createCar(Car car) async {
  var randomDoc = CarsRecord.collection.doc();
  var carRecord = CarsRecord.collection.doc(randomDoc.id);

  final carData = createCarsRecordData(
    car_id: randomDoc.id,
    name: car.name,
    user_id: car.userId,
  );

  await carRecord.set(carData);
}

// Create a Firestore document representing a bill

Future createFacture(Car car) async {
  var randomDoc = FacturesRecord.collection.doc();
  var factureRecord = FacturesRecord.collection.doc(randomDoc.id);

  final factureData = createFacturesRecordData();

  await factureRecord.set(factureData);
}
